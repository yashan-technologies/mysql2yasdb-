...
def build(args):
    ...
    if 'force' in targets:
        return True if builder.M2YBuilder().force_build() == 0 else False
    if not 'skip_check' in targets and not check(args):
        return False
    if not 'skip_test' in targets and not test(args):
        return False
    if 'clean' in targets and builder.M2YBuilder().clean() != 0:
        return False
    return True if builder.M2YBuilder().build() == 0 else False
